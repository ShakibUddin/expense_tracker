...
class Transaction {
    ...
    async createTransaction({
        ...
        const [results] = await db.execute(
            `INSERT INTO transactions(user_id, title, price_per_unit, description, category_id, quantity, total_price) VALUES(?,?,?,?,?,?,?)`,
            [userId, title, pricePerUnit, description, categoryId, quantity, pricePerUnit * quantity]
        );
        return results;
    }
    ...
    async updateTransaction({
        ...
        await db.execute(
            `UPDATE transactions SET title=?, price_per_unit=?, description=?, category_id=?, quantity=?, total_price=? WHERE id=?`,
            [title, pricePerUnit, description, categoryId, quantity, quantity * pricePerUnit, transactionId]
        );
    }
    ...
    async fetchExpensesByTimeframe({ userId }) {
        ...
        const [expenseCurrentDay] = await db.execute(`Select SUM(total_price) as total from transactions WHERE user_id=? AND DATE(created_at) = CURDATE()`, [userId])
        ...
        const [expenseCurrentWeek] = await db.execute(`Select SUM(total_price) as total from transactions WHERE user_id=? AND YEARWEEK(created_at, 1) = YEARWEEK(CURDATE(), 1)`, [userId])
        const [expenseCurrentMonth] = await db.execute(`Select SUM(total_price) as total from transactions WHERE user_id=? AND YEAR(created_at) = YEAR(CURDATE()) AND MONTH(created_at) = MONTH(CURDATE())`, [userId])
        const [expenseCurrentYear] = await db.execute(`Select SUM(total_price) as total from transactions WHERE user_id=? AND YEAR(created_at) = YEAR(CURDATE())`, [userId])
    ...
    async fetchExpenseOfAllCategories({userId}){
        const [results] =  await db.execute(`SELECT categories.id, categories.title, SUM(transactions.total_price) as total FROM transactions INNER JOIN categories ON transactions.category_id = categories.id WHERE transactions.user_id=? GROUP BY transactions.category_id`,[userId]);
